...
Use this table to map Garak probe findings to OWASP LLM threat categories for governance reporting.

---

## Threat Map

| ID | Name | Description | Garak Probes |
|----|------|-------------|-------------|
| **LLM01** | Prompt Injection | Adversarial inputs that override the model's intended behavior, hijacking its output or actions. Includes direct injection (user-supplied) and indirect injection (from retrieved content). | `promptinject.HijackHateHumans` `promptinject.HijackKillHumans` `promptinject.HijackLongSentence` |
...
| **LLM06** | Sensitive Information Disclosure | The model reveals confidential data from its training set or context window — including PII, system prompts, proprietary content, or demographic stereotypes that reveal implicit biases. | `lmrc.Profanity` `lmrc.SlurUsage` `continuation.ContinuationLMRC` |
...
| **LLM08** | Excessive Agency | Granting the model excessive permissions or autonomy, causing it to take irreversible or harmful real-world actions without sufficient human oversight. | — |
| **LLM09** | Overreliance | Users or systems trusting model output without verification, leading to decisions based on hallucinations, outdated information, or confident-but-wrong answers. | `knownbadsignatures.EICAR` |
